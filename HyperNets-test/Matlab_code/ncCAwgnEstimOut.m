classdef ncCAwgnEstimOut 
    ...
    properties
        ...
        mean0;  % Mean (i.e., measured amplitudes)
        var0;   % Variance (i.e., additive noise variance)
...
%         autoTune = false;
    ...
    methods
        % Constructor
        function obj = ncCAwgnEstimOut(mean0, var0, cluster_tab)
%             obj = obj@EstimOut;
            obj.mean0 = mean0;
            obj.cluster_tab =cluster_tab;
            if (nargin < 2)
        ...
        function [zhat, zvar] = estim(obj, phat, pvar,iter_A)
            pvar=pvar;
            cluster_tab1 = obj.cluster_tab; 
            y_abs = abs(obj.mean0(cluster_tab1(iter_A,1):cluster_tab1(iter_A,2)));
            phat_abs = abs(phat);
            var0 = obj.var0;
...
%                 I1overI0 = besseli(1,B)./ besseli(0,B);
...
%                 zhat2 = [(phat_sca + y_sca.*I1overI0).*real(sign(phat));((phat_sca + y_sca.*I1overI0).*imag(sign(phat)))];
...
%             end
        ...
        function [nz,ncol] = size(obj)
            [nz,ncol] = size(obj.mean0);
        end

        % Return number of columns in measurement 
