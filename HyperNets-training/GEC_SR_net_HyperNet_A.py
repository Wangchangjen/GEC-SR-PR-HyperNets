...
snr_train = 10.0 ** (snrdb_train/10.0)  #SNR mean
batch_size= 30
train_size = 120
epochs=10
itermax=10
...
snr_test = 10.0 ** (snrdb_test/10.0)  #SNR mean
...
# 1) Define the initial input damping value (Z_damping):
def Variable(shape):
    value =  np.array([[0.80611871, 0.71838841, 0.64077116, 0.56368683, 0.50416862, 0.44247516, 0.39285399, 0.34665258, 0.30251454, 0.3048238]]) # D = 10
    C = tf.constant_initializer(value)
    damping = tf.get_variable('damping', shape=shape, initializer = C,dtype='float64')  
    return damping


# 2) Amplitude observation: y = abs(z+w)   
def abq_y(y_input):
    abs_y = abs(y_input)
    return abs_y
...
def generate_data_iid_test(B,M,N,SNR,obj_rho_):  
    B_ = np.int(B/2)
    sigma2e = 10.**(-(SNR+(2*np.random.rand(B_,1)-1)*5)/10.) # SNR range [15,25]
...
# 5) Conjugate complex multiplication based on real value:
def complex_conj_mul_real(a,b,c):
...
# 5) Extrinsic part: mean & variance
...
def Train_batch(sess):
    ...
    for offset in range(packet):    
        ...
        for offset2 in range(packet2):
            ...
            _, b_loss,x_B_,v_B_,x_B2C_,v_B2C_,z_C2A_,v_C2A_,z_A2C_,v_A2C_,x_C2B_,v_C2B_,v_C_,z_C_,v_C_mean_,x_C_,z_A_,v_A_mean_,damping_,weights_\
            = sess.run([optimizer,cost,X_BB,v_B_mean,x_B2C,v_B2C,z_C2A,v_C2A,z_A2C,v_A2C,x_C2B,v_C2B,hat_varz_mean,hat_muz,hat_varx_mean,hat_mux,z_A,v_A_mean,damping,weights],\
            ...
            # print(sigmoid_fun(damping_[0]))
            # print(sigmoid_fun(damping_[5]))
            _loss.append(b_loss)
                       
    return _loss, x_B_, v_B_, x_B2C_, v_B2C_, z_C2A_, v_C2A_, z_A2C_, v_A2C_, x_C2B_, v_C2B_,damping_,weights_
...
def Train():
    ...
    with tf.Session() as sess:    
        ...
        for i in range(epochs):      
            train_loss, x_hat_train, v_B_,x_B2C_,v_B2C_,z_C2A_,v_C2A_,z_A2C_,v_A2C_,x_C2B_,v_C2B_,damping_, weights_= Train_batch(sess)
            if math.isnan(np.mean(train_loss)) == 0:
                    weight_file=weight_mat+'\weights_%d.mat' %i
                    Save(weight_file)          
            
            train_loss_list.append(train_loss)   
        print("\nTraining is finished.")
        saver.save(sess,"./checkpoint_dir/MyModel")
    return train_loss_list,damping_,v_B_,x_B2C_,v_B2C_,z_C2A_,v_C2A_,z_A2C_,v_A2C_,x_C2B_,v_C2B_ ,weights_  
...
def self_attention_Multi(Input,Input_Weights,Multi_num,d_k_size):
    
    Output_get={}
    for ii in range(Multi_num):
        Input_weight_WQ = Input_Weights['WQ_%d' %ii]
        Input_weight_WK = Input_Weights['WK_%d' %ii]
        Output_get['%d' %ii] = self_attention_get(Input,Input_weight_WQ,Input_weight_WK,d_k_size)
        
    for ii in range(Multi_num):
        if ii ==0:
            Output = Output_get['%d' %ii]
        else:
            Output = tf.concat([Output,Output_get['%d' %ii]],2)
    
    Output2 = tf.reduce_sum(Output*Input_Weights['h3'],axis=2)
...
with tf.Graph().as_default():
    ...
    for ii in range(Multi_num):
        if ii == 0:
            weights = {
                'WQ_%d' %ii: tf.Variable(tf.random_normal([1, KQ],dtype='float64')/np.sqrt((KQ)/2)),
                'WK_%d' %ii: tf.Variable(tf.random_normal([1, KQ],dtype='float64')/np.sqrt((KQ)/2)),
                'h1': tf.Variable(2*tf.ones([2*N, itermax],dtype='float64')/N),
                'h2': tf.Variable(3*tf.random_normal([N+1, 2*N],dtype='float64')/np.sqrt((N)/2)),
                'h3': tf.Variable(2*tf.random_normal([Multi_num],dtype='float64')/(Multi_num)),}
        else:
                weights['WQ_%d' %ii] = tf.Variable(tf.random_normal([1, KQ],dtype='float64')/np.sqrt((KQ)/2))
                weights['WK_%d' %ii] = tf.Variable(tf.random_normal([1, KQ],dtype='float64')/np.sqrt((KQ)/2))
    ...
    D_attention = self_attention_Multi(D_in,weights, Multi_num, d_k_size = np.float64(KQ))
    D2 = tf.nn.relu(tf.matmul(D_attention,weights['h2']))
    damping = tf.matmul(D2,weights['h1'])
    ...
    for t in range(itermax):      
       ...
       #1) damping_NN        
       v_A2C = (tf.sigmoid(tf.expand_dims(damping[:,t],-1)))*(v_A2C) + (1- tf.sigmoid(tf.expand_dims(damping[:,t],-1))) *(tf.divide(1,v_A2C_new))
       z_A2C = (tf.sigmoid(tf.expand_dims(damping[:,t],-1)))*(z_A2C) + (1- tf.sigmoid(tf.expand_dims(damping[:,t],-1))) *(z_A2C_new)   
       ...
       #1) damping_NN       
       v_B2C = (tf.sigmoid(tf.expand_dims(damping[:,t],-1)))*(v_B2C) + (1- tf.sigmoid(tf.expand_dims(damping[:,t],-1)))*(tf.divide(1,v_B2C_new))
       x_B2C = (tf.sigmoid(tf.expand_dims(damping[:,t],-1)))*(x_B2C) + (1- tf.sigmoid(tf.expand_dims(damping[:,t],-1)))*(x_B2C_new)
...
#    cost = tf.sum(each_iteration_loss)
    ...
    #Training DetNet
    train_loss_,damping_,v_B_,x_B2C_,v_B2C_,z_C2A_,v_C2A_,z_A2C_,v_A2C_,x_C2B_,v_C2B_, weights_=Train()
